...
Repository branch reviewed: `chore/migrate-supabase-to-neon`

Review basis: current source code, configuration, database migrations, tests, and Git history

## Executive summary

ANAI is a server-rendered ecommerce storefront for a Kenyan activewear and athleisure brand. It presents a curated product catalogue, maintains a browser-based bag and wishlist, reads current stock from Neon Postgres, accepts M-Pesa STK Push payments through Safaricom Daraja, sends transactional email through Brevo, and publishes search-engine and Google Merchant feeds.
...
- Commercial and operational data—variants, SKUs, prices used at checkout, and stock—lives in Neon Postgres.
...
| Language | TypeScript | Application, server, shared logic, and configuration |
...
| Database and backend service | Neon Postgres, PostgreSQL, Row Level Security | Catalogue variants, inventory, customers, orders, payments, newsletter, support, and notification state |
| Server SDK | `@neondatabase/serverless` | Private server-only parameterized SQL over Neon HTTP |
| Payments | Safaricom Daraja, Lipa na M-Pesa Online/STK Push | Kenyan mobile-money checkout and asynchronous payment callbacks |
| Email | Nuxt server utilities, Brevo transactional email API | Support notifications and idempotent paid-order customer/sales emails |
...
Nuxt application on AWS Amplify
  ...
  |-- Nitro APIs, middleware, sitemap, robots, and merchant feed
        ...
        |                        |
        v                        v
Neon Postgres              Safaricom Daraja
...
| `server/utils` | Private Neon client, server email jobs, M-Pesa client, callback recording, rate limiting, and feed creation |
...
| `database/neon` | Neon role, privilege, RLS, and verification SQL |
...
- The server normalizes and validates the request, retrieves active variants from Neon, and ignores client-side pricing.
...
Support and newsletter endpoints validate and normalize user data, apply request throttling, and write through the private Neon server client. Support submissions are saved before email is attempted, so an email-provider failure does not lose the request.

Private Nuxt server utilities use Brevo for email delivery. Customer-controlled values are escaped in HTML and plain-text fallbacks are produced. Paid-order delivery has a database-backed claim record, stale-claim recovery, Brevo idempotency keys, and sent/failed status tracking to reduce duplicate emails when callbacks or invocations repeat.
...
### Keep merchandising in code but make stock and checkout authoritative in Neon

The small catalogue can be reviewed and deployed with the site, and photo paths remain easy to control. Neon remains authoritative wherever correctness affects money or fulfilment. The trade-off is that matching catalogue data exists in two systems and depends on stable slugs and synchronization migrations.
...
If Neon successfully returns inventory and a product or colour is absent, it is treated as unavailable. Static stock is used only when live inventory cannot be loaded. This favors avoiding overselling over maximizing availability during inconsistent catalogue data.
...
- Private Neon, Brevo, and M-Pesa credentials remain in server runtime configuration.
...
Historical Supabase migrations remain alongside the application as schema records. Current Neon access-control SQL lives under `database/neon`, and `docs/neon-migration.md` documents safe branch testing and cutover. `docs/mpesa-daraja-setup.md` documents Daraja configuration.

### Continuous integration

GitHub Actions runs on pull requests and pushes to `main`, using Node.js 22. The application job installs with `npm ci`, runs the complete application check, and builds the Amplify artifact. A separate database job verifies only the approved disposable Neon endpoint. Concurrent runs for the same ref are cancelled when superseded.
...
- private pooled Neon URL in `NUXT_DATABASE_URL`;
...
- private Brevo API key, sender email, and support/sales notification recipients using `NUXT_` server variables.
...
- All 15 Node test files passed, including mocked Brevo payload tests; no real email or payment request was sent.
- The production Nuxt/Nitro build completed successfully.
- The disposable Neon branch passed read-only integrity and transactional least-privilege/RLS verification.

The test suite covers cart clamping, live-inventory behavior, checkout recovery and error messaging, M-Pesa cancellation and Buy Goods configuration, payment finalization requirements, database security/performance rules, merchant XML, navigation/canonical/sitemap behavior, branding, email escaping/idempotency, and global notifications.
